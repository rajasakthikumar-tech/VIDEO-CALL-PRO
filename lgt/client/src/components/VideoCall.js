...
function VideoCall() {
  ...
  useEffect(() => {
    ...
    // Fill white background so eraser reveals white, not transparent
    setTimeout(() => {
      if (!canvas) return;
      ...
      if (roomInfo?.whiteboardStrokes?.length) {
        replayStrokes(roomInfo.whiteboardStrokes);
      }
    }, 50);
  }, [showWhiteboard]);
  ...
  const setupSocketListeners = () => {
    ...
    socket.on('whiteboard-draw', (data) => {
      const canvas = wbCanvasRef.current;
      if (!canvas) return;
      drawStroke(canvas, data.x0, data.y0, data.x1, data.y1, data.color, data.size, data.isEraser);
    });
  ...
  const drawStroke = useCallback((canvas, x0, y0, x1, y1, color, size, isEraser) => {
    const ctx = canvas.getContext('2d');
    ctx.save();
    if (isEraser) {
  ...
  const wbDraw = useCallback((e) => {
    ...
    drawStroke(canvas, x0, y0, x1, y1, wbColor, wbSize, isEraser);
    wbLastPosRef.current = pos;
    // Emit to other participants
    if (socketRef.current?.connected) {
      socketRef.current.emit('whiteboard-draw', { x0, y0, x1, y1, color: wbColor, size: wbSize, isEraser });
    }
  }, [wbColor, wbSize, wbTool, drawStroke]);
  ...
  const replayStrokes = useCallback((strokes) => {
    ...
    strokes.forEach(s => drawStroke(canvas, s.x0, s.y0, s.x1, s.y1, s.color, s.size, s.isEraser));
  }, [drawStroke]);
          ...
          <div className="whiteboard-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                {['#000000','#e74c3c','#e67e22','#f1c40f','#2ecc71','#3498db','#9b59b6','#ffffff'].map(c => (
                  <button
                    key={c}
                    onClick={() => { setWbColor(c); setWbTool('pen'); }}
                    title={c}
                    style={{
                      width: 24, height: 24, borderRadius: '50%', background: c, border: wbTool === 'pen' && wbColor === c ? '3px solid #3498db' : '2px solid #ccc',
                ...
                <input
                  type="color"
                  value={wbColor}
                  onChange={e => { setWbColor(e.target.value); setWbTool('pen'); }}
                  title="Custom color"
                ...
                <select
                  ...
                  style={{ padding: '4px 6px', borderRadius: 4, border: '1px solid #ccc', cursor: 'pointer' }}
                >
                ...
                <button
                  ...
                  style={{
                    ...
                    background: wbTool === 'eraser' ? '#3498db' : '#ecf0f1',
                    color: wbTool === 'eraser' ? 'white' : '#333', fontWeight: 600
                  }}
                >
                  🧹 Eraser
                </button>
            ...
            <div className="whiteboard-content">
              <canvas
                ...
                width={800}
                height={560}
                style={{ cursor: wbTool === 'eraser' ? 'cell' : 'crosshair', display: 'block', borderRadius: 4 }}
